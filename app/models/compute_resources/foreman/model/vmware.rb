...
module Foreman::Model
  class Vmware < ComputeResource
    ...
    def save_vm(uuid, attrs)
      ...
      # volumes and interfaces are not part of vm.attributes so we have to set them seperately if needed
      ...
      if attrs.has_key?(:interfaces_attributes)
        vm.interfaces.each do |interface|
          ...
          interface.network = interface_attributes[:compute_attributes][:network]
        end
      end
      vm.save
    end
